...
void Page :: FromBinary (char* bits) {
	...
	for (int i = 0; i < numRecs; i++) {
		...
		tmpRecord->CopyBits(curPos, len);
		container.push_back(tmpRecord);
		curPos += len;
	}
}
...
void File :: AddPage (Page& addMe, off_t whichPage) {
	...
	//todo page change if update a page
	this->curLength = whichPage + 1;
//todo curlength
	delete [] bits;
}

void File::SetCurLength(off_t len) {
	this->curLength = len;
}
