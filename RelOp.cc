...
void Scan::createIndex(string table, int attributesId, Type eType) {
    // Initialize the BPlusTree module
	// Configuration parameters
	BPlusTree::TABLE_NAME = table;
	BPlusTree::Node::initialize();
    // Create a new tree
	BPlusTree::bRoot = new BPlusTree::Node();

    // Load session or build a new tree
    ifstream sessionFile(BPlusTree::SESSION_FILE + BPlusTree::TABLE_NAME);
	...
	Record recordTmp;
	while(file.GetNext(recordTmp) != -1) {
//		if (count % 5000 == 0) {
			cout << "Inserting " << count << endl;
			// Insert the object into file
			double key;
			//enum Type {Integer, Float, String, Name};todo only support int and double
			char * val1 = recordTmp.GetColumn(attributesId);
			if(eType == Integer) {
			...
			string dataString;
			dataString = to_string(file.indexOfPage);
			dataString += ",";
			int tmp = (file.indexOfRec - 1);
			dataString += to_string(tmp);
			BPlusTree::insert(BPlusTree::bRoot, BPlusTree::DBObject(key, dataString));
			// Update the counter
			count++;
			recordTmp.Nullify();
//		}
...
vector<BPlusTree::DBObject> Scan::searchRange(string table, double lowerLimit, double upperLimit) {
	BPlusTree::TABLE_NAME = table;
	BPlusTree::Node::initialize();
	BPlusTree::loadSession();
	return BPlusTree::windowQuery(BPlusTree::bRoot, lowerLimit, upperLimit);
}
...
void split2(const string& s, char delim,vector<string>& v) {
    auto i = 0;
...
bool Select::GetNext(Record& _record) {
    ifstream sessionFile(BPlusTree::SESSION_FILE + table);//todo table
//    cout<<"line 229 "<<BPlusTree::SESSION_FILE<<" "<<table<<endl;
    if (exists_file(BPlusTree::SESSION_FILE + table)) {
    	if(!retV.empty()) {
    		BPlusTree::DBObject tmp = *(retV.begin());
    		vector<string> vstring;
    		split2(tmp.getDataString(), ',', vstring);
    		int pageId = atoi(vstring[0].c_str());
    		int recordId = atoi(vstring[1].c_str());
    		if (Scan * sc = dynamic_cast<Scan *>(this->producer)) {
    			Page tmpP;
    			sc->file.file.GetPage(tmpP, pageId - 1);//todo start from 1;
    			Record* recordR = tmpP.container[recordId - 1];
    			_record.Swap(*recordR);
    		}
    		retV.erase(retV.begin());
    		return true;
    	}

    	if(retV.empty() && searchIndexOneTime) {
    		searchIndexOneTime = false;
    		int i;
			double d;
			string s;
			double key;
			...
			vector<double> ret;
			predicate.ExtractCNF2(*andList, schema, i, d, s, cmp, ret);
			if(ret.size() == 1) {//equals greater and less
				key = ret[0];
				if (Scan * sc = dynamic_cast<Scan *>(this->producer)) {
					if(cmp = Equals) {
						retV = sc->searchRange(table, key, key);
					} else if(cmp == LessThan) {
						retV = sc->searchRange(table, key, DBL_MAX);
					} else if(cmp == GreaterThan) {
						retV = sc->searchRange(table, DBL_MIN, key + 1);
					}
				}
			} else if(ret.size() == 2) {//range
				double small, big;
				if(ret[0] <= ret[1]) {
					small = ret[0];
					big = ret[1];
				...
				if (Scan * sc = dynamic_cast<Scan *>(this->producer)) {
					retV = sc->searchRange(table, small, big);
				}
			}
    	}
    	if(!retV.empty()) {
...
//			cout<<"left.size:"<<each->size()<<endl;
		...
		//			cout<<"rightcount "<<rightcount++<<endl;
					...
					if(predicateThread.andList[0].op == Equals) {
						if(predicateThread.andList[0].operand1 == Target::Right || predicateThread.andList[0].operand2 != Target::Right) {
												string value;
												...
												valueTmp = value;
											} else if(predicateThread.andList[0].operand2 == Target::Right || predicateThread.andList[0].operand1 != Target::Right) {
												string value;
												//enum Type {Integer, Float, String, Name};
												if(predicateThread.andList[0].attType == Type::Integer) {
...
bool Join::GetNext(Record& _record) {
	...
	while(1) {
		...
		leftSet.insert(tmp);
		if(predicate.andList[0].operand1 == Target::Left || predicate.andList[0].operand2 != Target::Left) {
			string value;
			//enum Type {Integer, Float, String, Name};
...
//				cout<<"line 443 "<<tmp->GetSize()<<"\n";
		...
		} else if(predicate.andList[0].operand2 == Target::Left || predicate.andList[0].operand1 != Target::Left) {
			string value;
			if(predicate.andList[0].attType == Type::Integer) {
...
//	cout<<"left join size:"<<leftSet.size()<<endl;
//	rightSet.clear();
	while(1) {
		Record* tmp = new Record();
...
//	cout<<"right join size:"<<rightSet.size()<<endl;
//	cout<<"predicate: "<<predicate.numAnds<<endl;
...
bool DuplicateRemoval::GetNext(Record& _record) {
	while (1) {
		...
		stringstream s;
		_record.print(s, schema);
		auto it = mapRecord.find(s.str());
		if(it == mapRecord.end()) {
			mapRecord[s.str()] = true;
			return true;
		}
	}
}
...
bool GroupBy::GetNext(Record& _record) {
	...
	if(isFirst == true) {
		...
		while(1) {
				...
				if(value == "5010" || value == "7510" || value == "2510" || value == "10") {
					cout<<"gmap value:"<<value<<endl;
				}
