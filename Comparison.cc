...
bool Comparison :: Run (Record& left, Record& right) {
	...
	string left1(left_bits);
//	cout<<"line 49:"<<left1<<endl;
	string right2(lit_bits);
//	cout<<"line 51:"<<right2<<endl;
...
//			if(strcmp("Customer#000070919", val1) == 0) {
//				int a = 0;
//				a = 1;
//			}
//			cout<<"line 94:"<<val1<<endl;
//			cout<<"line 94:"<<val2<<endl;
...
Type CNF::ExtractCNF2 (AndList& parseTree, Schema& schema, int& intR, double& doubleR, string& stringR, CompOperator& cmp,
		vector<double>& vd) {
	Type ret;
	numAnds = 0;
	int numFieldsInLiteral = 0;
	int recSize = 0;
	int attStart[MAX_ANDS];
	char recBits[PAGE_SIZE];
	char* recPos = recBits;
	...
	while (currCond != NULL) {
		...
		if (currCond->left->left->code == NAME) {
			...
			intR = atoi(currCond->left->left->value);

			vd.push_back((double) intR);

			if(currCond->left->code == EQUALS) {
				cmp = Equals;
			} else if(currCond->left->code == LESS_THAN) {
				cmp = LessThan;
...
//			cout<<"line 501:"<<currCond->left->left->value<<endl;

			doubleR =  (double)atof (currCond->left->right->value);

			vd.push_back((double) doubleR);
		...
		if (currCond->left->right->code == NAME) {
			...
			intR = atoi (currCond->left->right->value);
			...
			attStart[numFieldsInLiteral] = recSize;
			int cLen = sizeof(double);
//			cout<<"line 570:"<<currCond->left->left->value<<endl;
		...
		if (typeLeft != typeRight) {
			cerr <<"typeLeft :" <<typeLeft<<"typeRight "<<typeRight<< endl;
			cerr << "ERROR: Type mismatch for line 570" << currCond->left->left->value
