...
public:
	...
	Schema schema;
	CNF predicate;
	void createIndex(string table, int attributesId, Type eType);

	vector<BPlusTree::DBObject> searchRange(string table, double lowerLimit, double upperLimit);
...
public:
	int cnt = 0;
	bool searchIndexOneTime = true;
	AndList * andList;
	vector<BPlusTree::DBObject> retV;
...
public:
	unordered_map<string, bool> mapRecord;
	deque<Record*> dupSet;
